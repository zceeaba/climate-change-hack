...
def build_fao_data():
    employment = pd.read_csv(
        r'data/FAOSTAT/Employment_Indicators_E_All_Data_(Normalized)/Employment_Indicators_E_All_Data_(Normalized).csv')
    temperature_change = pd.read_csv(
        r'data/FAOSTAT/Environment_Temperature_change_E_All_Data_(Normalized)/Environment_Temperature_change_E_All_Data_(Normalized).csv')
    population = pd.read_csv(
        r'data/FAOSTAT/Population_E_All_Data_(Normalized)/Population_E_All_Data_(Normalized).csv')
    crops = pd.read_csv(
        r'data/FAOSTAT/Production_CropsProcessed_E_All_Data_(Normalized)/Production_CropsProcessed_E_All_Data_(Normalized).csv')
    land_cover = pd.read_csv(
        r'data/FAOSTAT/Environment_LandCover_E_All_Data_(Normalized)/Environment_LandCover_E_All_Data_(Normalized).csv')
    land_use = pd.read_csv(
        r'data/FAOSTAT/Environment_LandUse_E_All_Data_(Normalized)/Environment_LandUse_E_All_Data_(Normalized).csv')
    food_aid = pd.read_csv(
        r'data/FAOSTAT/Food_Aid_Shipments_WFP_E_All_Data_(Normalized)/Food_Aid_Shipments_WFP_E_All_Data_(Normalized).csv')
    food_security = pd.read_csv(
        r'data/FAOSTAT/Food_Security_Data_E_All_Data_(Normalized)/Food_Security_Data_E_All_Data_(Normalized).csv')
    ...
    employment_wide = employment.pivot(index = ('Area Code', 'Area', 'Source Code', 'Year'),
                                       columns = 'Indicator_Units')['Value']
    employment_wide.reset_index(inplace = True)

    temperature_change = temperature_change[temperature_change.Area == 'Niger']
    temperature_change[
        'Indicator_Units'] = temperature_change.Element + '_' + temperature_change.Unit
    ...
    temperature_change_wide = \
    temperature_change.pivot(index = ('Area Code', 'Area', 'Element Code', 'Year', 'Months'),
                             columns = 'Indicator_Units')['Value']
    ...
    population_wide = population.pivot(index = ('Area Code', 'Area', 'Element Code', 'Year'),
                                       columns = 'Indicator_Units')['Value']
    ...
    crops_wide = crops.pivot(index = ('Area Code', 'Area', 'Element Code', 'Item', 'Year'),
                             columns = 'Indicator_Units')['Value']
    ...
    land_cover_wide = \
    land_cover.pivot(index = ('Area Code', 'Area', 'Element Code', 'Item', 'Year'),
                     columns = 'Indicator_Units')['Value']
    ...
    land_use_wide = land_cover.pivot(index = ('Area Code', 'Area', 'Element Code', 'Item', 'Year'),
                                     columns = 'Indicator_Units')['Value']
    ...
    food_aid_wide = food_aid.pivot(index = ('Area Code', 'Area', 'Element Code', 'Item', 'Year'),
                                   columns = 'Indicator_Units')['Value']
    ...
    food_security_wide = \
    food_security.pivot(index = ('Area Code', 'Area', 'Element Code', 'Item', 'Year'),
                        columns = 'Indicator_Units')['Value']
    food_security_wide.reset_index(inplace = True)

    niger_data_long = pd.concat([employment, temperature_change, population, crops],
                                ignore_index = True)
    niger_data_wide = pd.merge(employment_wide, temperature_change_wide, how = 'outer',
                               on = ('Area Code', 'Area', 'Year'))
    niger_data_wide = pd.merge(niger_data_wide, population_wide, how = 'outer',
                               on = ('Area Code', 'Area', 'Year', 'Element Code'))
    niger_data_wide = pd.merge(niger_data_wide, temperature_change_wide, how = 'outer',
                               on = ('Area Code', 'Area', 'Year', 'Element Code'))
